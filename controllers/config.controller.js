...
const SiteTwo = require('../models/config/sitetwo.model');
...
module.exports = {
    ...
    // sitetwo
    viewSiteTwo: async (req, res) => {
        try {
            const sitetwo = await SiteTwo.find();
            const alertMessage = req.flash('alertMessage');
            const alertStatus = req.flash('alertStatus');
            const alert = {message: alertMessage, status: alertStatus};
            res.render('config/sitetwo/view_sitetwo', {
                title: 'Viana | SiteTwo',
                alert,
                sitetwo,
                page: 'SiteTwo'
            ...
            req.flash('alertMessage', `${error.message}`);
            req.flash('alertStatus', 'danger'); 
            res.redirect('/configuration/sitetwo');
        }
    },
    addSiteTwo: async (req, res) => {
        try {
            const {name, description, city, location} = req.body;
            await SiteTwo.create({
                name,
                description,
                city,
                location
    ...
    editSiteTwo: async (req, res) => {
        try {
            const {id, name, description, city, location} = req.body;
            const new_sitetwo = await SiteTwo.findOne({_id: id});
            new_sitetwo.name = name;
            new_sitetwo.description = description;
            new_sitetwo.city = city;
            new_sitetwo.location = location;
            await new_sitetwo.save();
            req.flash('alertMessage', 'Success update sitetwo');
            req.flash('alertStatus', 'success'); 
            res.redirect('/configuration/sitetwo');
        } catch (error) {
            req.flash('alertMessage', `${error.message}`);
    ...
    deleteSiteTwo: async (req, res) => {
        try {
            const {id} = req.params;
            const sitetwo = await SiteTwo.findOne({_id: id});
            if(sitetwo.cameraId.length != 0){
                req.flash('alertMessage', 'Cannot delete site, document is used in camera');
                req.flash('alertStatus', 'danger'); 
                res.redirect('/configuration/sitetwo');
            }else{
                await sitetwo.remove();
                req.flash('alertMessage', 'Success delete sitetwo');
                req.flash('alertStatus', 'success'); 
                res.redirect('/configuration/sitetwo');
            }
        } catch (error) {
    ...
    sitetwoById: async (req, res) => {
        try {
            const {id} = req.params;
            const sitetwo = await SiteTwo.findById(id).populate({
                path: 'cameraId',
                match: {status: 'active'}
            });
            res.status(200).send(sitetwo);
        } catch (error) {
            res.status(400).send(error);
        }
    },
